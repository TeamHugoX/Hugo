...
@hugo_cmd(pattern="promote ?(.*)", admins_only=True, type=["official", "manager"])
async def prmte(hg):
    xx = await eor(hg, get_string("com_1"))
    await hg.get_chat()
    user, rank = await get_uinfo(hg)
    rank = rank or "Admin"
    if not user:
        return await xx.edit(get_string("pro_1"))
    try:
        await hg.client.edit_admin(
            hg.chat_id,
            user.id,
            invite_users=True,
            ban_users=True,
        ...
        await eod(
            xx, get_string("pro_2").format(inline_mention(user), hg.chat.title, rank)
        )
...
async def dmote(hg):
    ...
    try:
        ...
        await eod(xx, get_string("de_2").format(inline_mention(user), hg.chat.title))
    except Exception as ex:
        return await xx.edit(f"`{ex}`")
...
async def bban(hg):
    user, reason = await get_uinfo(hg)
    if not user:
        return await eod(hg, get_string("ban_1"))
    if user.id in DEVLIST:
        return await eod(hg, get_string("ban_2"))
    try:
        await hg.client.edit_permissions(hg.chat_id, user.id, view_messages=False)
    except BadRequestError:
        return await eod(hg, get_string("ban_3"))
    except UserIdInvalidError:
        return await eod(hg, get_string("adm_1"))
    senderme = inline_mention(await hg.get_sender())
    userme = inline_mention(user)
    text = get_string("ban_4").format(userme, senderme, hg.chat.title)
    if reason:
...
async def uunban(hg):
    xx = await eor(hg, get_string("com_1"))
    if hg.text[1:].startswith("unbanall"):
        return
    user, reason = await get_uinfo(hg)
    if not user:
        return await xx.edit(get_string("unban_1"))
    try:
        await hg.client.edit_permissions(hg.chat_id, user.id, view_messages=True)
    except BadRequestError:
        return await xx.edit(get_string("adm_2"))
    except UserIdInvalidError:
        await xx.edit(get_string("adm_1"))
    sender = inline_mention(await hg.get_sender())
    text = get_string("unban_3").format(user, sender, hg.chat.title)
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
async def kck(hg):
    if "kickme" in hg.text:
        return
    ml = hg.text.split(" ", maxsplit=1)[0]
    xx = await eor(hg, get_string("com_1"))
    user, reason = await get_uinfo(hg)
    if not user:
    ...
    try:
        await hg.client.kick_participant(hg.chat_id, user.id)
    ...
    text = get_string("kick_4").format(
        inline_mention(user), inline_mention(await hg.get_sender()), hg.chat.title
    )
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
@hugo_cmd(pattern="tban ?(.*)", type=["official", "manager"])
async def tkicki(e):
    huh = e.text.split(" ")
    try:
        tme = huh[1]
...
@hugo_cmd(pattern="pin$", type=["official", "manager"])
async def pin(msg):
    if not msg.is_reply:
...
async def unp(hg):
    ...
    msg = None
    if hg.is_reply:
        msg = hg.reply_to_msg_id
    elif ch != "all":
        return await xx.edit(get_string("unpin_1").format(HNDLR))
    try:
        await hg.client.unpin_message(hg.chat_id, msg)
    except BadRequestError:
        return await xx.edit(get_string("adm_2"))
    except Exception as e:
...
async def fastpurgerme(purg):
    ...
    async for msg in purg.client.iter_messages(
        ...
        if len(msgs) == 100:
            await hugo_bot.delete_messages(chat, msgs)
            msgs = []
...
@hugo_cmd(pattern="pinned", type=["official", "manager"], groups_only=True)
...
async def autodelte(hg):
    match = hg.pattern_match.group(1)
    if not match or match not in ["24h", "7d", "1m", "off"]:
        return await eor(hg, "`Please Use in Proper Format..`", time=5)
    if match == "24h":
        tt = 3600 * 24
    elif match == "7d":
        tt = 3600 * 24 * 7
        ...
        tt = 0
    try:
        await hg.client(SetHistoryTTLRequest(hg.chat_id, period=tt))
    except ChatNotModifiedError:
        return await eor(
            hg, f"Auto Delete Setting is Already same to `{match}`", time=5
        )
    await eor(hg, f"Auto Delete Status Changed to `{match}` !")
