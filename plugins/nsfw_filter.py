...
from pyHugo.dB.nsfw_db import is_nsfw, nsfw_chat, rem_nsfw

from . import HNDLR, eor, events, udB, hugo_bot, hugo_cmd


@hugo_cmd(pattern="addnsfw ?(.*)", admins_only=True)
async def addnsfw(e):
...
@hugo_cmd(pattern="remnsfw", admins_only=True)
async def remnsfw(e):
    rem_nsfw(e.chat_id)
        ...
        if nsfw:
            ...
            if NWARN.get(e.sender_id):
                ...
                if count < 3:
                    ...
                    return await hugo_bot.send_message(
                        chat,
                ...
                if "mute" in action:
                    ...
                    try:
                        await hugo_bot.kick_participant(chat, e.sender_id)
                        await hugo_bot.send_message(
                            chat,
                            f"NSFW Warn 3/3 to [{e.sender.first_name}](tg://user?id={e.sender_id})\n\n**Action Taken** : {action}",
                        )
