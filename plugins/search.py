...
from pyHugo.functions.google_image import googleimagesdownload
...
async def gitsearch(event):
    ...
    hg = await async_searcher(url, re_json=True)
    try:
        uname = ult["login"]
        uid = ult["id"]
...
@hugo_cmd(pattern="reverse$")
async def reverse(event):
    reply = await event.get_reply_message()
    if not reply:
        return await eor(event, "`Reply to an Image`")
    hg = await eor(event, get_string("com_1"))
