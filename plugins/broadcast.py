...
from pyHugo.dB.broadcast_db import *

from . import HNDLR, LOGS, eor, get_string, udB, hugo_bot, hugo_cmd
...
async def forw(event):
    ...
    for channel in channels:
        try:
            await hugo_bot.forward_messages(int(channel), previous_message)
            sent_count += 1
            await x.edit(
                f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
            )
...
async def sending(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message:
            ...
            for channel in channels:
                try:
                    await hugo_bot.send_message(int(channel), previous_message)
                    sent_count += 1
                    await x.edit(
                        f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
                    )
