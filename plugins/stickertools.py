...
async def pack_kangish(_):
    ...
    if len(_.text) > 9:
        ...
        _packname = f"Hugo Kang Pack By {_.sender_id}"
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await hugo_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await hugo_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent hugo_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with hugo_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await hugo_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                await conv.get_response()
                await hugo_bot.send_read_acknowledge(conv.chat_id)
...
async def hgdestroy(event):
    hg = await event.get_reply_message()
    if not (ult and hg.media and "animated" in mediainfo(ult.media)):
        return await eor(event, get_string("sts_2"))
    await event.client.download_media(hg, "hugo.tgs")
    xx = await eor(event, get_string("com_1"))
    await bash("lottie_convert.py hugo.tgs json.json")
    ...
    await bash("lottie_convert.py json.json hugo.tgs")
    await event.client.send_file(
        event.chat_id,
        file="hugo.tgs",
        force_document=False,
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/hugo_blank.png")
    if ik.endswith(".tgs"):
        await event.client.download_media(reply, "ult.tgs")
        await bash("lottie_convert.py hg.tgs json.json")
        with open("json.json") as json:
            jsn = json.read()
        jsn = jsn.replace("512", "2000")
        open("json.json", "w").write(jsn)
        await bash("lottie_convert.py json.json hg.tgs")
        file = "ult.tgs"
        os.remove("json.json")
