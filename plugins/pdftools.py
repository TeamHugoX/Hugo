...
from pyHugo.functions.tools import four_point_transform
...
async def pdfseimg(event):
    ...
    if not msg:
        ...
        for o in range(ok):
            ...
            with open(os.path.join("hg.png"), "wb") as f:
                pw.write(f)
            await event.reply(
                file="hg.png",
            )
        ...
        os.remove(pdfp)
        try:
            await event.reply(file="hg.png")
        except PhotoSaveFileInvalidError:
            await event.reply(file="hg.png", force_document=True)
        os.remove("hg.png")
