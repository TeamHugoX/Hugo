...
from pyHugo.misc._assistant import asst_cmd
...
@hugo_cmd(pattern="kickme$", fullsudo=True)
async def leave(hg):
    me = asst.me if hg.client._bot else hugo_bot.me
    await eor(hg, f"`{me.first_name} has left this group, bye!!.`")
    await hg.client(LeaveChannelRequest(hg.chat_id))
...
async def stats(
    ...
    try:
        ...
        ct = 0
    try:
        sp = await hugo_bot(GetAllStickersRequest(0))
        sp_count = len(sp.sets)
    except BaseException:
        sp_count = 0
    full_name = inline_mention(await hugo_bot.get_me())
...
@hugo_cmd(pattern="paste( (.*)|$)", type=["official", "manager"])
async def _(event):
...
async def _(hg):
    ...
    to_add_users = hg.pattern_match.group(1)
    if not hg.is_channel and hg.is_group:
        for user_id in to_add_users.split(" "):
            try:
                ...
                await xx.edit(f"Successfully invited `{user_id}` to `{hg.chat_id}`")
            except Exception as e:
                await xx.edit(str(e))
    else:
        for user_id in to_add_users.split(" "):
...
async def telegraphcmd(event):
    match = event.pattern_match.group(1) or "Hugo"
    reply = await event.get_reply_message()
    if not reply:
...
@hugo_cmd(pattern="suggest", type=["official", "manager"])
