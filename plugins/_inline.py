...
from pyHugo.functions.helper import gen_chlog, time_formatter, updater
from pyHugo.misc import CMD_HELP, owner_and_sudos
from pyHugo.misc._assistant import callback, in_pattern
...
TLINK = INLINE_PIC or "https://telegra.ph/file/1dbf193322538a9c19b21.jpg"
...
SUP_BUTTONS = [
    [
        Button.url("• ʀᴇᴘᴏ", url="https://github.com/TeamHugoX/Hugo"),
        Button.url("sᴜᴘᴘᴏʀᴛ •", url="t.me/HugoSupport"),
    ],
]
...
async def inline_alive(o):
    ...
    await o.answer(RES, switch_pm="👥 HUGO PORTAL", switch_pm_param="start")


@in_pattern("hgd", owner=True)
async def inline_handler(event):
    ...
    if INLINE_PIC:
        ...
        result = await event.builder.article(
            title="Hugo Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(e):
    button = [
        ...
        [
            Button.switch_inline(
                "ᴘɪsᴛᴏɴ ᴇᴠᴀʟ",
                query="run javascript console.log('Hello Hugo')",
                same_peer=True,
            ),
            Button.switch_inline(
                "ᴏʀᴀɴɢᴇ ғᴏx🦊",
...
async def addon(event):
    if HELP.get("Addons"):
        ...
        await event.answer(
            f"• ᴛʏᴘᴇ {HNDLR}setredis ADDONS True\n ᴛᴏ ɢᴇᴛ ᴀᴅᴅᴏɴs ᴘʟᴜɢɪɴs",
            cache_time=0,
            alert=True,
        )
...
async def on_vc_plg_callback_query_handler(event):
    ...
    buttons.append(
        [
            Button.inline("« ʙᴀᴄᴋ", data="vc_helper"),
        ]
    )
    try:
        if str(event.query.user_id) in owner_and_sudos():
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Hugo Op", text=txt, link_preview=False, buttons=btn)
    ]
