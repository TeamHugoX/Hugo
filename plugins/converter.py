...
from . import bash, downloader, eor, get_paste, get_string, udB, hugo_cmd, uploader
...
async def _(e):
    ...
    await bash(f"wget {nn} -O resources/extras/hugo.png")
...
async def _(event):
    ...
    await event.reply(file=input_str, thumb="resources/extras/hugo.png")
    await xx.delete()
    os.remove(input_str)
