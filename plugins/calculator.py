...
from . import Button, asst, callback, get_string, in_pattern, udB, hugo_cmd
...
thgd = [Button.inline(f"{x}", data=f"calc{x}") for x in m]
lst = list(zip(thgd[::4], thgd[1::4], thgd[2::4], thgd[3::4]))
lst.append([Button.inline("=", data="calc=")])


@hugo_cmd(pattern="calc")
async def icalc(e):
    udB.delete("calc")
    if e.client._bot:
