...
from pyHugo.dB.logusers_db import *
from pyHugo.dB.pmpermit_db import *
...
if sett == "True":
    ...
    async def autoappr(e):
        ...
        try:
            await hugo_bot.edit_folder(miss.id, folder=0)
        except BaseException:
            pass
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE") == "True":
                try:
                    await hugo_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await hugo_bot.inline_query(
                        ...
                        _to_delete[user.id] = await hugo_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await hugo_bot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await hugo_bot(BlockRequest(user.id))
                await hugo_bot(ReportSpamRequest(peer=user.id))
                name = await hugo_bot.get_entity(user.id)
...
@hugo_cmd(pattern="block ?(.*)", fullsudo=True)
...
async def list_approved(event):
    ...
    for i in all:
        try:
            name = (await hugo_bot.get_entity(i)).first_name
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await hugo_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user_name = (await hugo_bot.get_entity(uid)).first_name
        except BaseException:
            user_name = ""
        await event.edit(
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    await hugo_bot(BlockRequest(uid))
    try:
        user_name = (await hugo_bot.get_entity(uid)).first_name
    except BaseException:
        user_name = ""
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    await hugo_bot(UnblockRequest(uid))
    try:
        user_name = (await hugo_bot.get_entity(uid)).first_name
    except BaseException:
        user_name = ""
...
async def ytfuxist(e):
    try:
        ...
        await hugo_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• Hugo •", switch_pm_param="start")
