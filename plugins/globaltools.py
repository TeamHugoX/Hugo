...
from pyHugo.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from pyHugo.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
from pyHugo.functions.tools import create_tl_btn, format_btn, get_msg_button
...
@hugo_cmd(pattern="gpromote ?(.*)", fullsudo=True)
async def _(e):
...
@hugo_cmd(pattern="gdemote ?(.*)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1)
    hugo_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await hugo_bot.get_entity(user)
        except BaseException:
            return await eor(e, f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in hugo_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@hugo_cmd(pattern="ungban ?(.*)", fullsudo=True)
async def _(e):
...
@hugo_cmd(pattern="gban ?(.*)", fullsudo=True)
async def _(e):
...
@hugo_cmd(pattern="g(admin|)cast ?(.*)", fullsudo=True)
...
@hugo_cmd(pattern="gucast ?(.*)", fullsudo=True)
...
@hugo_cmd(pattern="gkick ?(.*)", fullsudo=True)
async def gkick(e):
...
@hugo_cmd(pattern="gmute ?(.*)", fullsudo=True)
async def _(e):
...
@hugo_cmd(pattern="ungmute ?(.*)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    for i in users:
        try:
            name = (await hugo_bot.get_entity(int(i))).first_name
        except BaseException:
            name = i
...
@hugo_cmd(pattern="gblacklist")
async def blacklist_(event):
    await gblacker(event, "add")


@hugo_cmd(pattern="ungblacklist")
...
async def gblacker(event, type_):
    ...
    try:
        ...
        pass
    try:
        chat_id = (await hugo_bot.get_entity(chat)).id
