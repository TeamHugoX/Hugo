...
from . import eor, get_string, hugo_bot, hugo_cmd


@hugo_cmd(pattern="qrcode ?(.*)")
async def cd(e):
    ...
    pfp = await e.client.get_profile_photos(hugo_bot.uid)
    img = "resources/extras/hugoproject.png"
    if len(pfp) >= 1:
