...
from pyHugo.dB.botchat_db import tag_add, who_tag
...
if udB.get("TAG_LOG"):
    ...
    async def idk(e):
        ...
        if chat and msg:
            try:
                await hugo_bot.send_message(chat, e.message, reply_to=msg)
...
async def _(e):
    ...
    y = await hugo_bot.get_entity(int(wah))
...
async def when_hgd_added_to_chat(event):
    user = await event.get_user()
    chat = await event.get_chat()
    if not (user and user.is_self):
        return
