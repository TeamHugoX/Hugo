...
from . import asst, eor, events, get_string, udB, hugo_bot, hugo_cmd


@hugo_bot.on(events.NewMessage())
async def _(e):
    ...
    for ys in y:
        try:
            await hugo_bot.send_message(int(ys), e.message)
        except Exception as ex:
            await asst.send_message(int(udB["LOG_CHANNEL"]), str(ex))


@hugo_cmd(pattern="shift (.*)")
async def _(e):
...
async def source(e):
    ...
    try:
        ...
        try:
            y = int((await hugo_bot.get_entity(x)).id)
...
@hugo_cmd(pattern="listdest")
async def list_all(event):
    hugo_bot = event.client
    x = await eor(event, get_string("com_1"))
    ...
    for channel in channels:
        name = ""
        try:
            name = (await hugo_bot.get_entity(int(channel))).title
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
