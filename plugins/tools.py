...
from pyHugo.functions.tools import metadata
...
from . import hugo_cmd, uploader


@hugo_cmd(pattern="tr", type=["official", "manager"])
async def _(event):
...
@hugo_cmd(pattern="bots ?(.*)", groups_only=True, type=["official", "manager"])
async def _(hg):
    mentions = "**Bots in this Chat**: \n"
    input_str = hg.pattern_match.group(1)
    to_write_chat = await hg.get_input_chat()
    chat = None
    if not input_str:
        chat = to_write_chat
    else:
        ...
        try:
            chat = await hg.client.get_entity(input_str)
        except Exception as e:
            return await eor(hg, str(e))
    try:
        async for x in hg.client.iter_participants(
            chat,
...
async def _(hg):
    try:
        input = hg.text.split(" ", maxsplit=1)[1]
    except IndexError:
        return await eor(hg, "`Input some link`", time=5)
    await eor(hg, "[ㅤㅤㅤㅤㅤㅤㅤ](" + input + ")", link_preview=False)
...
async def _(e):
    ...
    if a.document and a.document.mime_type == "audio/mpeg":
        ...
        try:
            ...
            bbbb = "resources/extras/hugo.png"
            im = cv2.imread(bbbb)
            dsize = (320, 320)
            output = cv2.resize(im, dsize, interpolation=cv2.INTER_AREA)
            cv2.imwrite("img.png", output)
...
async def _(e):
    ...
    try:
        ...
        with io.BytesIO(str.encode(text)) as out_file:
            ...
            await e.reply(
                f"`{e.text}`", file=out_file, thumb="resources/extras/hugo.png"
            )
        await e.delete()
