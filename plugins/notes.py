...
from pyHugo.dB.notes_db import add_note, get_notes, list_note, rem_note
...
@hugo_cmd(pattern="addnote ?(.*)", admins_only=True)
async def an(e):
...
@hugo_cmd(pattern="remnote ?(.*)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1)).lower()
    chat = e.chat_id
    if not wrd:
...
@hugo_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
