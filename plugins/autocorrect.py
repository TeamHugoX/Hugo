...
from . import HNDLR, Redis, eor, get_string, udB, hugo_bot, hugo_cmd


@hugo_cmd(pattern="autocorrect", fullsudo=True)
async def acc(e):
...
@hugo_bot.on(events.NewMessage(outgoing=True, func=lambda x: x.text))
