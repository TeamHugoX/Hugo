...
from . import download_file, eor, get_string, udB, hugo_cmd


@hugo_cmd(pattern="autopic ?(.*)")
async def autopic(e):
...
@hugo_cmd(pattern="stoppic$")
async def stoppo(hg):
    gt = udB.get("AUTOPIC")
    if gt != "True":
        return await eor(hg, get_string("autopic_4"), time=5)
    udB.set("AUTOPIC", "None")
    await eor(hg, get_string("autopic_5"), time=5)
