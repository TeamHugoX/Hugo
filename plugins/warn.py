...
from pyHugo.dB.warn_db import add_warn, reset_warn, warns

from . import eor, inline_mention, udB, hugo_cmd
...
async def warn(e):
    ...
    if reply:
        ...
        try:
            user = e.text.split()[1]
            if user.startswith("@"):
                ok = await hugo_bot.get_entity(user)
                user = ok.id
            else:
                user = int(user)
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await hugo_bot.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
                return await eor(e, "`Something Went Wrong.`", time=5)
        elif "kick" in action:
            try:
                await hugo_bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await eor(e, "`Something Went Wrong.`", time=5)
        elif "mute" in action:
            try:
...
@hugo_cmd(pattern="setwarn ?(.*)", type=["official", "manager"])
async def warnset(e):
