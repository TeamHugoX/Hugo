...
@hugo_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
    ...
    if not stderr and not stdout:
        OUT += "**• OUTPUT:**\n`Success`"
    if len(OUT) > 4096:
        hgd = OUT.replace("`", "").replace("**", "").replace("__", "")
        with io.BytesIO(str.encode(hgd)) as out_file:
...
@hugo_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    ...
    if len(final_output) > 4096:
        hgd = final_output.replace("`", "").replace("**", "").replace("__", "")
        with io.BytesIO(str.encode(hgd)) as out_file:
...
@hugo_cmd(pattern="cpp", only_devs=True)
async def doie(e):
    ...
    open("cpp-hugo.cpp", "w").write(match)
    m = await bash("g++ -o CppHugo cpp-hugo.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1] != "":
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-hugo.cpp")
            if os.path.exists("CppHugo"):
                os.remove("CppHugo")
