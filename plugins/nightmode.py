...
from pyHugo.dB.night_db import *
from telethon.tl.functions.messages import EditChatDefaultBannedRightsRequest
from telethon.tl.types import ChatBannedRights

from . import LOGS, eor, hugo_bot, hugo_cmd


@hugo_cmd(pattern="nmtime ?(.*)")
...
@hugo_cmd(pattern="addnm ?(.*)")
async def add_grp(e):
    pat = e.pattern_match.group(1)
    if pat:
        try:
            add_night((await hugo_bot.get_entity(pat)).id)
...
@hugo_cmd(pattern="remnm ?(.*)")
async def rem_grp(e):
    pat = e.pattern_match.group(1)
    if pat:
        try:
            rem_night((await hugo_bot.get_entity(pat)).id)
...
@hugo_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = night_grps()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await hugo_bot.get_entity(x)
...
async def open_grp():
    ...
    for chat in chats:
        try:
            ...
            await hugo_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
